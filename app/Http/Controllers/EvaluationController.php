...
class EvaluationController extends Controller {
	...
	public function create()
	{
        ...
        public function forcharts() 
        {
            ...
            $whogotB = DB::select("SELECT stuid FROM evaluation_mark WHERE total > '74' and total < '85'");
            $whogotC = DB::select("SELECT stuid FROM evaluation_mark WHERE total > '64' and total < '75'");
            $whogotD = DB::select("SELECT stuid FROM evaluation_mark WHERE total > '49' and total < '65'");
            $whogotF = EvaluationMarks::where('total','<','50')->count();
            
            $proposalprsentcomp = EvaluationMarks::where('completedassesment','proposalprsent')->count();
            $proposalreportcopm = EvaluationMarks::where('completedassesment','proposalreport')->count();
            $srs = EvaluationMarks::where('completedassesment','srs')->count();
            $prototype = EvaluationMarks::where('completedassesment','prototype')->count();
            $midprsent = EvaluationMarks::where('completedassesment','midprsent')->count();
            $midreport = EvaluationMarks::where('completedassesment','midreport')->count();
            $finalprsent = EvaluationMarks::where('completedassesment','finalprsent')->count();
            $finalreport = EvaluationMarks::where('completedassesment','finalreport')->count();
            $viva = EvaluationMarks::where('completedassesment','viva')->count();
            $other = EvaluationMarks::where('completedassesment','other')->count();
            
            
            $data = array("whogotA" => $whogotA, "whogotB" => $whogotB, "whogotC" => $whogotC, "whogotD" => $whogotD,
                "whogotF" => $whogotF, "proposalprsentcomp" => $proposalprsentcomp, "proposalreportcopm" => $proposalreportcopm,
                "srs" => $srs, "prototype" => $prototype, "midprsent" => $midprsent, "midreport" => $midreport,
                "finalprsent" => $finalprsent, "finalreport" => $finalreport, "viva" => $viva,
                "other" => $other);
        ...
        public function settingsupdate()
        {
            ...
            if(Request::get('tab2') == "tab2")
            {
                ...
                if(Request::get('mem1') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'proposalpresent' => Request::get('mem1')
                                ]);
                }
                
                if(Request::get('mem2') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'proposalreport' => Request::get('mem2')
                                ]);
                }
                
                if(Request::get('mem3') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'srsdoc' => Request::get('mem3')
                                ]);
                }
                
                if(Request::get('mem4') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'srsstatus' => Request::get('mem4')
                                ]);
                }
                
                if(Request::get('mem5') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'protoprsent' => Request::get('mem5')
                                ]);
                }
                
                if(Request::get('mem6') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'midpresent' => Request::get('mem6')
                                ]);
                }
                
                if(Request::get('mem7') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'midreport' => Request::get('mem7')
                                ]);
                }
                
                if(Request::get('mem8') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'finalprsent' => Request::get('mem8')
                                ]);
                }
                
                if(Request::get('mem9') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'finalreport' => Request::get('mem9')
                                ]);
                }
                
                if(Request::get('mem10') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'finalstatusdoc' => Request::get('mem10')
                                ]);
                }
                
                if(Request::get('mem11') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'viva' => Request::get('mem11')
                                ]);
                }
                
                if(Request::get('mem12') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'researchbook' => Request::get('mem12')
                                ]);
                }
                
                if(Request::get('mem13') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'researchpaper' => Request::get('mem13')
                                ]);
                }
                
                if(Request::get('mem114') != null)
                {                
                    setting::where('id', '1')
                        ->update([
                            'website' => Request::get('mem14')
                                ]);
                }
                return redirect('settings')->with("success", 'Successfully Changed!');
            }                        
