            ...
            <nav class="navbar-default navbar-static-side" role="navigation">
                <div class="sidebar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        {{--</li>--}}
                        <li>
                    <a href="/taskSupervisor"><i class="fa fa-th-large"></i> <span class="nav-label">Task Allocation</span> <span class="fa"></span></a>
                        </li>
                        <li>
                                    ...
                                    <li>
                                        <ul class="dropdown-messages" style="height: 300px; overflow: auto;">
                                            @for($id=0;$id<count(Session::get('notification.BelongsToPanelMember'));$id++)
                                                ...
                                                echo $now->diff($rpc->created_at)->format("%h");
                                                ?>hr ago</small>
                                                            <strong > </strong>
                                                            {{$rpc->text }}<strong> by {{ $rpc->from_id }}</strong>. <br>
                                                            <small class="text-muted"><?php $now = new DateTime();
                                                                echo $now->diff($rpc->created_at)->format("%a");
                                                ?> days ago at <?php echo date_format($rpc->created_at, 'g:i A'); ?> - <?php echo $newDate = date("Y/m/d", strtotime($rpc->created_at)); ?>
                                                            </small>
                                                        </div>
